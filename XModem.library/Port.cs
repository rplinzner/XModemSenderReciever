...
namespace XModem.library
{
    public class Port
    {
        ...
        private readonly SerialPort _selectedPort;
        //Table from: http://www.sunshine2k.de/coding/javascript/crc/crc_js.html
        private static ushort[] _crc16LookupTable =
        {
        ...
        public byte Read()
        {
            return (byte)_selectedPort.ReadByte();
        }
        public void Write(byte[] buffer, int offset, int count)
        {
            _selectedPort.Write(buffer, offset, count);
        }
        ...
        public short GetCrcSum(byte[] data)
        {
            ...
            {
                result = (short)((result << 8) ^ _crc16LookupTable[((result >> 8) ^ b) & 0x00FF]);
            }

            return result;
        }

        public byte GetCkSum(byte[] data)
        {
            byte result = 0;
            foreach (var b in data)
            {
